...
def gen_sqf16_data():
    #data:
    raw_sqf16 = pd.read_csv("https://www1.nyc.gov/assets/nypd/downloads/excel/analysis_and_planning/stop-question-frisk/sqf-2016.csv",low_memory=False)
...
def gen_chile_data():
  raw_chile = pd.read_csv('C:/Users/Elias/esktop/UnfairGAN Stuff/Chile_dataset',sep=";")
...
work_dir_ = os.path.join('C:/Users/Elias/Desktop/UnfairGAN Stuff/', dataset_name)
...
if(dataset_name=='chile'):
    eval_gen = pd.read_csv('C:/Users/Elias/Desktop/UnfairGAN Stuff/'+dataset_name+'/data-2019-04-09-08-25.csv',index_col=0)
    eval_gen = sample_from_prob(eval_gen)
elif(dataset_name == 'cmp'):
    eval_gen = pd.read_csv('C:/Users/Elias/Desktop/UnfairGAN Stuff/'+dataset_name+'/data-2019-02-19-09-34.csv',index_col=0)
    eval_gen = sample_from_prob(eval_gen)
elif(dataset_name == 'schufa'):
    eval_gen = pd.read_csv('C:/Users/Elias/Desktop/UnfairGAN Stuff/'+dataset_name+'/data-2019-05-07-08-25.csv',index_col=0)
    eval_gen = sample_from_prob(eval_gen)
elif(dataset_name == 'sqf'):
    eval_gen = pd.read_csv('C:/Users/Elias/Desktop/UnfairGAN Stuff/'+dataset_name+'/data-2019-02-05-14-34.csv',index_col=0)
    eval_gen = sample_from_prob(eval_gen)
...
def add_nan_noise(df_old,missing):
  ...
  poss_locs = np.array(list(itertools.product(range(len(df.index)),range(len(df.columns)))))
  tuples = poss_locs[np.random.choice(range(len(poss_locs)),sample_size,replace=False)]
  for s in tuples:
...
def add_col_nan_noise(df_old,missing):
  df = df_old.copy()
  samp_cols = df.columns.to_series().sample(np.int8(len(df.columns)*(1.5-missing)))
  df[samp_cols] = np.nan
  return df

def impute_missing(df):
  imputer = MissForest(max_depth=20,max_iter=100)
  return imputer.fit_transform(df)

def create_combined_set(data_real,data_gen,missing,n_percent,full_noise=False):
    ...
    df_real_imp = impute_missing(combined)[0:len(df_real_x.index),:]    
...
class CV_pred_protected():
  ...
  def create_var_predset(self,cols):
    ...
    data_x_o,data_x_g = self.data_orig.drop(cols,axis=1),self.data_gen.drop(cols,axis=1)
...
#         m1_round = np.int8(m1_pred > .5)
  ...
  def cv_pred_protected_var(self,data_x_o,data_x_g,data_y_o,data_y_g,v):
    ...
    for df1,df2 in zip(skf.split(data_x_o,data_y_o),skf.split(data_x_g,data_y_g)):
      ...
      print('Starting missing data eval...')
      for i in np.arange(1.0,0.0,-.1): #from 90% to 10% 
        ...
        if(i==1):
            samp_cols = data_x_o.columns.to_series().sample(np.int8(len(data_x_o.columns)*i+.5))
            data_miss = [data_g[0][samp_cols],data_r[1][samp_cols],data_g[2],data_r[3]]
            ...
            for _ in range(random_iter):
                samp_cols = data_x_o.columns.to_series().sample(np.int32(len(data_x_o.columns)*i+.5))
                data_miss = [data_g[0][samp_cols],data_r[1][samp_cols],data_g[2],data_r[3]]
                fpr_,tpr_,roc_auc_= self.get_cfm_auc(data_miss,v)  
                fpr_miss.append(fpr_)
                tpr_miss.append(tpr_)
            ...
            for j in [0.5,.3,.1]:
                for _ in range(random_iter):
                    ...
                    data_imp_miss = create_combined_set(data_r,data_g,missing=i,n_percent=j)
                    fpr_,tpr_,roc_auc_= self.get_cfm_auc(data_imp_miss,v)  
...
prot_ev = CV_pred_protected(eval_orig_c,eval_gen_c,prot_dict)
prot_ev.evaluate(10)
#prot_ev.summary()
